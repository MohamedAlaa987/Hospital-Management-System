...
namespace WindowsFormsApp12
{
    public partial class Form11 : Form
    {
        ...
        List<Room> arr1 = new List<Room>();
        Dictionary<int, string> dic = new Dictionary<int, string>();
        database db = new database();
        SqlConnection cnn;
        private string patquery = "select PID,Pname from patient";
        private string roomquery = "select * from room";
        ...
        private void initmenu()
        {
            ...
            label24.Text = moveToToolStripMenuItem.DropDownItems[8].Text;
        }
        ...
        private void Form4_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                retrievedata(roomquery, "room");


                db.closeconnection();
                foreach(Room x in arr1) { guna2ComboBox1.Items.Add(x.Rname); }
        ...
        private void retrievedata(string query,string key )
        {
            ...
            else if (key == "room")
            {
                while (data.Read())
                {
                    ...
                    Room ro = new Room();
                    ro.RID1 = data.GetInt32(0);
                    ro.Rname = data.GetString(1);
                    ro.Rcost = data.GetInt32(3);
                    ro.PID1 = data.GetInt32(4);
                    ro.Rtype = data.GetString(2);
                    arr1.Add(ro);
        ...
        private void checkInput(string input )
        {
            ...
            {
                foreach(Room y in arr1)
                {
                    if (y.Rname == input&&x.PID1==y.PID1)
                    {
        ...
        private void  DisplayResult(int index,Patient x,Room y)
        {
           guna2DataGridView1.Rows[index].Cells[0].Value = y.Rname;
           guna2DataGridView1.Rows[index].Cells[1].Value = x.Pname;
           guna2DataGridView1.Rows[index].Cells[2].Value = y.Rcost;
           guna2DataGridView1.Rows[index].Cells[3].Value = y.Rtype;
